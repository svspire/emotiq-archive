...
(defun write-dotfile-stream (stream nodelist graphID)
  (let ((mapname (random-name)))
    ...
    (format stream " dim=2;~%") ; do NOT increase this beyond 2, or Graphviz will take exponentially longer
    (format stream " outputorder=\"edgesfirst\";~%")
    (format stream " ratio=\"compress\";~%")
    (format stream " overlap=false;~%")
    (format stream " repulsiveforce=5;~%")
    (format stream " start=1;~%") ; random seed
    (format stream " K=1.0;~%")
